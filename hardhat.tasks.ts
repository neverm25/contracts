...
import fs from "fs";
// load contracts json
const contractsJson = fs.readFileSync("./contracts.json", "utf8");
...
async function loadCfg() {
    ...
    let contracts = allContracts[chainId];
    if( ! contracts ) {
        new Error(`contracts not found for chainId ${chainId}`);
    }
    if( ! config ) {
        new Error(`config not found for chainId ${chainId}`);
    }
...
task("price", "get oracle price").setAction(async () => {
    ...
    const poolAddress = cfg.VARA_KAVA;
